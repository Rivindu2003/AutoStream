...
function startServer(port = PORT) {
 ...
 <p><a href="/test-tv">Test TV Detection</a></p>
      ...
      // Redirect path-based configure requests to query-based configure requests
      const pathBasedConfigureMatch = pathname.match(/^\/([^\/]+)\/configure$/);
      if (pathBasedConfigureMatch) {
        const configurationPath = pathBasedConfigureMatch[1];
        const decodedPath = decodeURIComponent(configurationPath);
        const separator = decodedPath.includes('|') ? '|' : '&';
        const parts = decodedPath.split(separator);
        ...
        res.writeHead(302, { 'Location': `/configure?${queryParams.toString()}` });
        return res.end();
      }
